...
class ZAutoEncoder(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, device):
        super(ZAutoEncoder, self).__init__()
    ...
    def forward(self, x):
        h = self.encoder(x)
        rex = self.decoder(h).squeeze()
        return rex


class InformationEnhancement(nn.Module):
    def __init__(self, channel, hidden, device):
        super(InformationEnhancement, self).__init__()
