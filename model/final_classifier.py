...
class CLASSIFIER:
    def __init__(self, model, _train_X, _train_Y, _test_seen_X, _test_seen_Y, _test_novel_X, _test_novel_Y, seenclasses,
                 novelclasses,
                 _nclass, device, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, use=None,
                 ignore=None, train_only=False, test_only=False, do_nothing=False):
    ...
    def compute_per_class_acc(self, test_label, predicted_label, nclass):
        ...
        target_classes = torch.arange(0, nclass, out=torch.LongTensor()).to(
            self.device)  # changed from 200 to nclass on 24.06.
        predicted_label = predicted_label.to(self.device)
